...
def rep_with_upper(password):
    ...
    while a < len(res):
        if random.randint(0, 1) == 1:
            ...
            a = a+1
        a = a+1
    newPassword = ''.join(res)
    return newPassword


def swap_letters(test):
    m = []
    m[:] = test
    m[0], m[1], m[len(m)-1], m[len(m)-2] = m[len(m)-1], m[len(m)-2], m[0], m[1]

    newest_password = ''.join(m)
    return newest_password


letter = input("Please enter a letter to be searched")
...
d = passGen(user_number)
print(d)
print(swap_letters(rep_with_upper(d)))
print(search_letter())
